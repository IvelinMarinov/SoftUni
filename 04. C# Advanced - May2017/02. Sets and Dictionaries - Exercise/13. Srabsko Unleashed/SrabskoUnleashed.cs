    ...
    public class СръбскоUnleashed
    {
        public static void Main()
        {
            ...
            while (inputString != "End")
            {
                ...
                var remainingInput = input[1]
                    .Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries)
                    .ToArray();

                var ticketPrice = int.Parse(remainingInput[remainingInput.Length - 2]);
                var ticketsCount = int.Parse(remainingInput[remainingInput.Length - 1]);
                var venue = string.Empty;

                for (int i = 0; i < remainingInput.Length - 2; i++)
                {
